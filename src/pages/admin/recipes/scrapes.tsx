...
export default async function Pages__admin__recipes__scrapes({ ctx }: RequestInfo): Promise<React.JSX.Element> {
	...
	const rows = enrichedScrapes.map(u => ({ ...u, viewUrl: `/admin/recipes/${u.id}/view` })).filter(row => row.source);
	return (
