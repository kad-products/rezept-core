 ...
 */
export async function _saveIngredientSeason(formData: IngredientSeasonFormInput): Promise<ActionState<IngredientSeasonsDBRead>> {
	...
	try {
		...
		if (!parsed.success) {
			return errorResponse<IngredientSeasonsDBRead>(parsed.error.flatten().fieldErrors, 400);
		}
		if (parsed.data.id) {
			const updatedIngredientSeason = await updateIngredientSeason(parsed.data.id, parsed.data, userId, requestInfo.ctx.logger);
			return successResponse<IngredientSeasonsDBRead>(updatedIngredientSeason);
		}
		const createdIngredientSeason = await createIngredientSeason(parsed.data, userId, requestInfo.ctx.logger);
		return successResponse<IngredientSeasonsDBRead>(createdIngredientSeason);
	} catch (error) {
		requestInfo.ctx.logger.error('Failed to save ingredient season', { error });
		return errorResponse<IngredientSeasonsDBRead>(error, 500, 'Failed to save ingredient season');
	}
}
