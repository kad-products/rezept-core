...
describe('_submitRecipeSearch', () => {
	...
	describe('successful search', () => {
		it('returns success with matched recipes', async () => {
			const result = await _submitRecipeSearch({ growingZoneId });
		...
		it('calls searchRecipes with growingZoneId and term', async () => {
			await _submitRecipeSearch({ growingZoneId, recipeSearchTerm: 'tomato' });

			expect(searchRecipes).toHaveBeenCalledWith({ growingZoneId, term: 'tomato' }, expect.anything());
		});

		it('passes undefined term when recipeSearchTerm is omitted', async () => {
			await _submitRecipeSearch({ growingZoneId });

			expect(searchRecipes).toHaveBeenCalledWith({ growingZoneId, term: undefined }, expect.anything());
		});
