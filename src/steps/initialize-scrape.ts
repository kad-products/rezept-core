...
export async function initializeScrape(parsedBody: unknown, userId: string, logger: RzLogger): Promise<RecipeScrapeDBRead> {
	...
	try {
		...
		throw new RzStepError(400, 'Failed to initialize recipe scrape', `Error writing scrape to R2: ${err}`, true, err);
	}
	...
	try {
		...
		throw new RzStepError(400, 'Failed to initialize recipe scrape', `Error creating scrape record: ${err}`, true, err);
	}
}
